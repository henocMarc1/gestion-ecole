-- FIX INTELLIGENT : Fonction check_overdue_payments qui s'adapte automatiquement
-- Cette version vérifie quelle colonne existe et l'utilise
...
DECLARE
    ...
    has_total_amount_column BOOLEAN;
    has_paid_amount_column BOOLEAN;
    has_amount_paid_column BOOLEAN;
BEGIN
    -- Détecter quelles colonnes existent
    SELECT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'tuition_fees' AND column_name = 'amount'
    ) INTO has_amount_column;
    
    SELECT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'tuition_fees' AND column_name = 'total_amount'
    ) INTO has_total_amount_column;
    
    SELECT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'tuition_fees' AND column_name = 'paid_amount'
    ) INTO has_paid_amount_column;
    
    SELECT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'tuition_fees' AND column_name = 'amount_paid'
    ) INTO has_amount_paid_column;

    -- Si on n'a ni amount ni total_amount, retourner vide avec un message d'erreur
    IF NOT has_amount_column AND NOT has_total_amount_column THEN
        RAISE EXCEPTION 'La table tuition_fees ne contient ni "amount" ni "total_amount". Colonnes disponibles: %',
            (SELECT string_agg(column_name, ', ') FROM information_schema.columns WHERE table_name = 'tuition_fees');
    END IF;

    -- Version qui s'adapte selon les colonnes disponibles
    IF has_amount_column AND has_paid_amount_column THEN
        -- Cas 1: amount + paid_amount
        RETURN QUERY
        SELECT 
            s.id AS student_id,
            ...
            COALESCE(
                (SELECT (tf_inner.amount - COALESCE(tf_inner.paid_amount, 0))
        ...
        WHERE 
            ...
            AND EXISTS (
                ...
                AND (tf_check.paid_amount IS NULL OR tf_check.paid_amount < tf_check.amount)
            );
    
    ELSIF has_total_amount_column AND has_amount_paid_column THEN
        -- Cas 2: total_amount + amount_paid
        RETURN QUERY
        SELECT 
            s.id AS student_id,
            (s.first_name || ' ' || s.last_name) AS student_name,
    ...
    ELSE
        RAISE EXCEPTION 'Configuration de colonnes non supportée. Envoyez la liste des colonnes de tuition_fees.';
    END IF;
END;
$$ LANGUAGE plpgsql;
